...
getEnvironment = (branch) ->
  if branch == 'master' or branch == 'master-missu3'
    return 'Production'
  else if branch == 'staging' or branch == 'staging-missu3'
    return 'Staging'
  else
    return 'Local'
...
module.exports = (robot) ->
  robot.router.post "/hubot/circleci/say", (req, res) ->
    ...
    if step_failed
      fields.push
        value: "--> #{step_failed.name}: failed"
        short: false

    if room? == false
      fields.push
        value: "use sempre suas iniciais como nome do branch (ex: mt-meu-branch) ainda não tem PR aberto?, altere agora. 'git branch -m mt-novo-nome'"
        short: false

    payload =
      channel: room or '#missu-dev'
      content:
        author_name: "[#{body.branch}] Build ##{body.build_num} - #{body.outcome.toUpperCase()}"
        author_link: body.build_url
        fallback: "[#{body.branch}] Build ##{body.build_num} - #{body.outcome.toUpperCase()}"
